...
from DescriptorCreator.molecule_formats import convert_2Dsdf_to_3Dxyz
...
class EASMolPreparation():
    ...
    def generate_3Dxyz(self, smi, name):
        ...
        if AllChem.EmbedMolecule(self.rdkit_mol, ps) == -1:
            print(f'Embed failed for {name} with SMILES: {smi}; will use openbabels 2d-->3d generator')
            EMBED_COMPLETED = False
            # raise Exception(f'Embed failed for {name} with SMILES: {smi}')
        else:
        ...
        if EMBED_COMPLETED:
            ...
            w = Chem.SDWriter(f'{self.mol_calc_path}/{name}.2d.sdf')
            w.write(self.rdkit_mol)
            w.close()
            convert_2Dsdf_to_3Dxyz(f'{self.mol_calc_path}/{name}.2d.sdf', self.xyz_file_path)
    ...
    def calc_CM5_charges(self, smi, name='pred_mol', optimize=False, save_output=False):
        ...
        # Run xTB calc
        if optimize:
            cmd = f'{XTBHOME}/bin/xtb --gfn 1 {self.xyz_file_path} --opt --lmo --chrg {chrg} --uhf {spin}'
        else:
